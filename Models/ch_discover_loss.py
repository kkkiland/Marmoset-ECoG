...
class DynamicalContrastiveLoss(torch.nn.Module):
    ...
    def forward(self, scores, attn_mask, norm_matrix, return_cosine=True):
        b = scores.shape[0]
        n_vars = scores.shape[-1]
        #print("NaN in norm_matrix:", torch.isnan(norm_matrix).any().item())       # 输出是否存在NaN
        ...
        #print("NaN in all_scores:", torch.isnan(all_scores).any().item()) 
