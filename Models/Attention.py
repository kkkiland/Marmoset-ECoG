...
class ChannelAttention(nn.Module):
    def __init__(self, channels, reduction=8):
        ...
        self.max_pool = nn.AdaptiveMaxPool1d(1)  # 加入最大池化增强特征
        self.fc = nn.Sequential(
            nn.Linear(256 * 2, channels // reduction),  # 双通道信息融合
    ...
    def forward(self, x):
        ...
        scale = self.fc(combined).unsqueeze(-1)  # [B,C,1]
        return x * scale.expand_as(x), scale.squeeze(-1)  # 同时返回缩放后的特征和注意力权重
