...
import math, torch
from Models.ch_discover_loss import DynamicalContrastiveLoss
...
class c_Attention(nn.Module):
    ...
    def forward(self, x, attn_mask=None):
        ...
        #print("NaN in scores:", torch.isnan(scores).any().item())
        q_norm = torch.norm(q, dim=-1, keepdim=True)
        k_norm = torch.norm(k, dim=-1, keepdim=True)
        norm_matrix = torch.einsum('bhid,bhjd->bhij', q_norm, k_norm)
        #print("NaN in norm_matrix:", torch.isnan(norm_matrix).any().item())  
        if attn_mask is not None:
            ...
            #print("NaN in scores:", torch.isnan(scores).any().item())
            #print("1 not in attn_mask:", not (attn_mask == 1).any().item())
            dynamical_contrastive_loss, cosine_matrix = self.dynamicalContranstiveLoss(scores, attn_mask, norm_matrix)
        else:
        ...
        return self.to_out(out), attn, dynamical_contrastive_loss, cosine_matrix
...
class c_Transformer(nn.Module):  ##Register the blocks into whole network
    ...
    def forward(self, x, attn_mask=None):
        ...
        for attn, ff in self.layers:
            x_n, attn, dcloss, cosine_matrix = attn(x, attn_mask=attn_mask)
            total_loss += dcloss
            x = x_n + x
            x = ff(x) + x
        dcloss = total_loss / len(self.layers)
        return x, attn, dcloss, cosine_matrix
...
class Trans_C(nn.Module):
    ...
    def forward(self, x, attn_mask=None):
        ...
        x, attn, dcloss, cosine_matrix = self.transformer(x, attn_mask)
        x = self.dropout(x)
        x = self.mlp_head(x).squeeze()
        return x, dcloss, cosine_matrix  # ,attn
